...
import com.cinema.javafx.view.builder.DescriptionTabBuilder;
...
public class CardContainer {
    ...
    public CardContainer(CardsController cardsController, Movie movie) throws IOException {
        ...
        card = FXMLLoader.load(getClass().getResource("/fxml/card.fxml"));
        init();
    }
    ...
    private void init() {
        ...
        trailer.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> cardsController.getRootController().runTrailer(movie.getTrailer()));
        Button play = (Button)((AnchorPane)((BorderPane)((TabPane)((AnchorPane) card.getChildren().get(1)).getChildren().get(0)).getTabs().get(0).getContent()).getBottom()).getChildren().get(1);
        play.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> cardsController.getRootController().runPlayer(movie));
